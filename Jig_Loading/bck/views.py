...
class InitJigLoad(APIView):
	...
	def get(self, request, *args, **kwargs):
		...
		# MULTI-MODEL: empty_hooks must account for ALL models, not just primary
		if multi_model_flag and multi_model_allocation:
			total_multi_model_qty = sum(m['allocated_qty'] for m in multi_model_allocation)
			empty_hooks = max(0, effective_capacity - total_multi_model_qty)
			logging.info(f"[MULTI_MODEL_HOOKS] total_allocated={total_multi_model_qty}, effective={effective_capacity}, empty={empty_hooks}")
		elif loaded_cases_qty > 0:
