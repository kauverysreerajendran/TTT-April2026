...
def brass_reject_check_tray_id_simple(request):
    ...
    # ✅ FIX: Only block if the tray is occupied by a DIFFERENT lot.
    #    If the tray belongs to the SAME lot (lot accepted from IS to
    #    Brass QC), the tray is part of this lot's flow and must be allowed.
    # -----------------------------------------------------------------
    is_occupied_in_is_by_other_lot = IPTrayId.objects.filter(
        tray_id=tray_id,
        rejected_tray=True,
        delink_tray=False
    ).exclude(
    ...
    if is_occupied_in_is_by_other_lot:
        print(f"[BRASS_REUSE] ❌ Tray {tray_id} is occupied in IS by a different lot (rejected_tray=True, delink_tray=False)")
        return JsonResponse({
            'exists': True,
            'valid_for_rejection': False,
    ...
    # ✅ Same-lot tray in IS: allowed — log for traceability
    is_same_lot_in_is = IPTrayId.objects.filter(
        tray_id=tray_id,
        lot_id=current_lot_id,
        rejected_tray=True,
        delink_tray=False
    ).exists()
    if is_same_lot_in_is:
        print(f"[BRASS_REUSE] ✅ Tray {tray_id} belongs to same lot {current_lot_id} in IS — allowing reuse")
