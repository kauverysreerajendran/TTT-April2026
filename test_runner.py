...
def setup_test_data():
    ...
    plating, _ = Plating_Color.objects.get_or_create(plating_color='Test_Black', defaults={'plating_color_internal': 'B'})
    version, _ = Version.objects.get_or_create(version_name='Test_V1', defaults={'version_internal': 'V1'})
    category, _ = Category.objects.get_or_create(category_name='Test_Cat')
    vendor, _ = Vendor.objects.get_or_create(vendor_name='Test_Vendor', defaults={'vendor_internal': 'TV'})
    ...
    model_master, _ = ModelMasterCreation.objects.get_or_create(
        model_no='TEST-001', batch_id='BATCH-001',
        defaults={
...
def test_normal_duplicate_blocking():
    ...
    if response.status_code == 409:
        ...
        print("FAILED: Duplicate should have been blocked")
        return False
