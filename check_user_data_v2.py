...
# Filtering for partial accept submissions using the boolean field
...
for sub in recent:
    ...
    print(f"  Status: Partial Accept")
    
    # Find accept children
    accept_children = IS_PartialAcceptLot.objects.filter(parent_lot_id=sub.lot_id)
    for child in accept_children:
        print(f"  ✅ Accept child: {child.new_lot_id} (qty={child.accepted_qty})")
