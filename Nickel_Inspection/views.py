...
def _get_input_source(jig_unload_obj):
    ...
    if not names:
        ...
        for raw_cid in jig_unload_obj.combine_lot_ids or []:
            ...
            tray = (
                TrayId.objects.filter(lot_id=cid)
                .select_related("batch_id__location")
                .first()
            )
...
class NQ_PickTableView(APIView):
    ...
    def get_dynamic_tray_capacity(self, tray_type_name):
        ...
        try:
            if tray_type_name:
                if tray_type_name.strip().lower() in [
                    "nr",
                    "normal",
                    "normal tray",
                    "normal-16",
            ...
            if tray_type_name:
                ...
                if name in ["nr", "normal", "normal tray"]:
                    return 20
            ...
            tray_type = TrayType.objects.filter(
                tray_type__iexact=tray_type_name
            ).first()
    ...
    def get(self, request):
        ...
        queryset = (
            JigUnloadAfterTable.objects.select_related(
        ...
        queryset = queryset.filter(
            ...
            | Q(
                rejected_nickle_ip_stock=True, nq_onhold_picking=True
            )  # Rejected but on hold
        ).order_by("-created_at", "-lot_id")

        print(
            "All lot_ids in queryset:", list(queryset.values_list("lot_id", flat=True))
        )
        ...
        for jig_unload_obj in page_obj.object_list:
            ...
            data = {
                ...
                "version__version_name": (
                    jig_unload_obj.version.version_name
                    if jig_unload_obj.version
                    else ""
                ),
                ...
                "last_process_module": jig_unload_obj.last_process_module
                or "Jig Unload",
            ...
            if not images and data["combine_lot_ids"]:
                ...
                print(
                    "🔄 NQ View - No ModelMaster images, trying combine_lot_ids fallback"
                )

                first_lot_id = (
                    data["combine_lot_ids"][0] if data["combine_lot_ids"] else None
                )
        ...
        for data in master_data:
            ...
            if total_IP_accpeted_quantity and total_IP_accpeted_quantity > 0:
                ...
                jig_unload_obj = JigUnloadAfterTable.objects.filter(
                    lot_id=lot_id
                ).first()
...
class NickelQcRejectTableView(APIView):
    ...
    def get(self, request):
        ...
        nickel_rejection_total_qty_subquery = (
            Nickel_QC_Rejection_ReasonStore.objects.filter(
                lot_id=OuterRef("lot_id")
            ).values("total_rejection_quantity")[:1]
        )
        ...
        for obj in page_obj.object_list:
            ...
            data = {
                ...
                "plating_color": (
                    obj.plating_color.plating_color if obj.plating_color else ""
                ),
                "polish_finish": (
                    obj.polish_finish.polish_finish if obj.polish_finish else ""
                ),
                "version__version_name": (
                    obj.version.version_name if obj.version else ""
                ),
                "vendor_internal": "",  # Not available in JigUnloadAfterTable
                "location__location_name": _get_input_source(obj),
                "tray_type": obj.tray_type or "",
                "tray_capacity": (
                    self.get_dynamic_tray_capacity(obj.tray_type)
                    if obj.tray_type
                    else 0
                ),
            ...
            if obj.plating_stk_no:
                ...
                if len(plating_stk_no) >= 4:
                    ...
                    try:
                        ...
                        print(
                            f"❌ Nickel Reject View - Error fetching ModelMaster: {e}"
                        )
            ...
            if not images and obj.combine_lot_ids:
                ...
                if first_lot_id:
                    ...
                    total_stock_obj = TotalStockModel.objects.filter(
                        lot_id=first_lot_id
                    ).first()
            ...
            if stock_lot_id:
                ...
                try:
                    ...
                    data["nickel_rejection_total_qty"] = data.get(
                        "nickel_rejection_total_qty", 0
                    )
...
def nickel_qc_get_rejection_details(request):
    ...
    try:
        ...
        reason_store = (
            Nickel_QC_Rejection_ReasonStore.objects.filter(lot_id=unload_lot_id)
            .order_by("-id")
            .first()
        )
        ...
        if reason_store and reason_store.batch_rejection:
            ...
            tray_scans = Nickel_QC_Rejected_TrayScan.objects.filter(
        ...
        try:
            ...
            if stock_lot_ids:
                ...
                for stock_lot_id in stock_lot_ids:
                    ...
                    rejected_tray_ids = list(
                        NickelQcTrayId.objects.filter(
                            lot_id=stock_lot_id, rejected_tray=True
                    ...
                    if rejected_tray_ids:
                        ...
                        updated_count_nickel = NickelQcTrayId.objects.filter(
                            lot_id=stock_lot_id, rejected_tray=True
                        ).update(delink_tray=True, rejected_tray=False)
                        ...
                        print(
                            f"✅ Delinked {updated_count_nickel} trays in NickelQcTrayId for lot {stock_lot_id}"
                        )

                        print(
                            f"✅ Delinked {updated_count_original} trays in TrayId for lot {stock_lot_id}"
                        )
                ...
                for tray_id in tray_ids:
                    ...
                    delink_tray_obj = NickelQcTrayId.objects.filter(
                        tray_id=tray_id
                    ).first()
                ...
                return JsonResponse(
                    {"success": False, "error": "No stock_lot_ids or tray_ids provided"}
                )
    ...
    def post(self, request):
        ...
        try:
            ...
            if not obj:
                ...
                return JsonResponse(
                    {"success": False, "error": "LOT not found."}, status=404
                )
            ...
            if missing_qty not in [None, ""]:
                ...
                try:
                    ...
                    return Response(
                        {
                            "success": False,
                            "error": "Missing quantity must be an integer",
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                ...
                if missing_qty > display_accepted_qty:
                    ...
                    return Response(
                        {
                            "success": False,
                            "error": f"Missing quantity must be less than or equal to display accepted quantity ({display_accepted_qty}).",
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
    ...
    def create_nickel_tray_instances(self, lot_id):
        ...
        try:
            ...
            for tray in verified_trays:
                ...
                nickel_tray = NickelQcTrayId.objects.filter(
                    tray_id=tray.tray_id, lot_id__isnull=True
                ).first()

                if nickel_tray:

                    print(
                        f"🔄 Updating NickelQcTrayId with empty lot_id for tray_id: {tray.tray_id}"
                    )
            ...
            print(
                f"❌ [create_nickel_tray_instances] Error creating/updating NickelQcTrayId instances: {str(e)}"
            )
    ...
    def post(self, request):
        ...
        try:
            ...
            if not lot_id:
                ...
                return JsonResponse(
                    {"success": False, "error": "Missing lot_id"}, status=400
                )
    ...
    try:
        ...
        if tray_obj and hasattr(tray_obj, "tray_type"):
            ...
            try:
                ...
                if (
                    total_stock
                    and hasattr(total_stock, "batch_id")
                    and total_stock.batch_id
                ):
        ...
        if tray_type_info and "NORMAL" in tray_type_info and raw_capacity == 16:
            ...
            print(
                f"🔧 [Tray Type Correction] Normal-16 → Normal-20 for lot_id: {lot_id}"
            )
        ...
        if raw_capacity in capacity_corrections:
            ...
            print(
                f"🔧 [Tray Capacity Correction] {raw_capacity} → {corrected_capacity} for lot_id: {lot_id}"
            )
...
def brass_get_tray_capacity_for_lot(request):
    ...
    try:
        ...
        print(
            f"🔍 [brass_get_tray_capacity_for_lot] Getting tray capacity for lot_id: {lot_id}"
        )
        ...
        if total_stock:
            ...
            if hasattr(total_stock, "batch_id") and total_stock.batch_id:
                ...
                if hasattr(batch_obj, "tray_capacity") and batch_obj.tray_capacity:
                    ...
                    corrected_capacity = apply_tray_capacity_correction(
                        raw_tray_capacity, None, lot_id
                    )

                    print(
                        f"✅ Found tray_capacity from ModelMasterCreation: {raw_tray_capacity} → Corrected: {corrected_capacity}"
                    )
        ...
        try:
            ...
            if (
                model_creation
                and hasattr(model_creation, "tray_capacity")
                and model_creation.tray_capacity
            ):
                ...
                print(
                    f"✅ Found tray_capacity from direct ModelMasterCreation lookup: {raw_tray_capacity} → Corrected: {corrected_capacity}"
                )
        ...
        if tray_objects.exists():
            ...
            for tray in tray_objects:
                ...
                if (
                    hasattr(tray, "tray_capacity")
                    and tray.tray_capacity
                    and tray.tray_capacity > 0
                ):
                    ...
                    corrected_capacity = apply_tray_capacity_correction(
                        raw_tray_capacity, tray, lot_id
                    )

                    print(
                        f"✅ Found tray_capacity from TrayId: {raw_tray_capacity} → Corrected: {corrected_capacity}"
                    )
        ...
        if total_stock:
            ...
            print(
                f"   - TotalStockModel exists: batch_id = {getattr(total_stock.batch_id, 'batch_id', 'None') if total_stock.batch_id else 'None'}"
            )

            if total_stock.batch_id:

                print(
                    f"   - ModelMasterCreation tray_capacity = {getattr(total_stock.batch_id, 'tray_capacity', 'None')}"
                )
        ...
        for mmc in all_mmc:
            ...
            print(
                f"     - batch_id: {mmc.batch_id}, tray_capacity: {getattr(mmc, 'tray_capacity', 'None')}"
            )
...
class NQDeleteBatchAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            print(
                f"🔍 [NQDeleteBatchAPIView] Deleting stock lot with ID: {stock_lot_id}"
            )

            if not stock_lot_id:

                return JsonResponse(
                    {"success": False, "error": "Missing stock_lot_id"}, status=400
                )
            ...
            if not obj:
                ...
                return JsonResponse(
                    {"success": False, "error": "Stock lot not found"}, status=404
                )
    ...
    def get(self, request):
        ...
        try:
            ...
            if not lot_id:
                ...
                return Response(
                    {"success": False, "error": "Missing lot_id parameter"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if not trays.exists():
                ...
                return Response(
                    {"success": False, "error": f"No trays found for lot {lot_id}"},
                    status=status.HTTP_404_NOT_FOUND,
                )
            ...
            for i, tray in enumerate(tray_list_sorted):
                ...
                print(
                    f"[DELINK DEBUG] Step {i+1}: tray_id={tray_id}, tray_qty={tray_qty}, remaining_missing={remaining_missing}"
                )
                ...
                if remaining_missing >= tray_qty:
                    ...
                    print(
                        f"[DELINK DEBUG] Delinking full tray {tray_id} (qty {tray_qty})"
                    )
                    ...
                    print(
                        f"[DELINK DEBUG] Top tray is {tray_id}: original_qty={tray_qty}, delinked_qty={remaining_missing}, remaining_qty_in_tray={remaining_qty_in_tray}"
                    )
            ...
            if (
                remaining_missing == 0
                and len(delink_trays) > 0
                and len(tray_list) > len(delink_trays)
                and top_tray is None
            ):
    ...
    def post(self, request):
        ...
        try:
            ...
            for delink_tray_id in delink_tray_ids:
                ...
                brass_delink_tray_obj = NickelQcTrayId.objects.filter(
                    tray_id=delink_tray_id, lot_id=lot_id
                ).first()
                ...
                ip_delink_tray_obj = JigUnload_TrayId.objects.filter(
                    tray_id=delink_tray_id, lot_id=lot_id
                ).first()
                ...
                if ip_delink_tray_obj:
                    ...
                    print(
                        f"✅ Delinked IPTrayId tray: {delink_tray_id} for lot: {lot_id}"
                    )

                # JigUnload_TrayId - Mark as delinked

                jig_unload_delink_tray_obj = JigUnload_TrayId.objects.filter(
                    tray_id=delink_tray_id, lot_id=lot_id
                ).first()
                ...
                if jig_unload_delink_tray_obj:
                    ...
                    print(
                        f"✅ Delinked JigUnload_TrayId tray: {delink_tray_id} for lot: {lot_id}"
                    )

                # TrayId - Remove from lot completely

                trayid_delink_tray_obj = TrayId.objects.filter(
                    tray_id=delink_tray_id, lot_id=lot_id
                ).first()
            ...
            if top_tray_id and top_tray_qty is not None:
                ...
                print(
                    f"[TOP TRAY] Updating tray: {top_tray_id} with qty: {top_tray_qty}"
                )

                # Update NickelQcTrayId for top tray

                top_tray_obj = NickelQcTrayId.objects.filter(
                    tray_id=top_tray_id, lot_id=lot_id
                ).first()
                ...
                if top_tray_obj:
                    ...
                    top_tray_obj.save(
                        update_fields=["top_tray", "tray_quantity", "delink_tray"]
                    )

                    print(
                        f"✅ Updated NickelQcTrayId top tray: {top_tray_id} to qty: {top_tray_qty}"
                    )

            # 3. Reset other trays (not delinked or top tray) to full capacity

            other_trays_brass = NickelQcTrayId.objects.filter(lot_id=lot_id).exclude(
            ...
            for tray in other_trays_brass:
                ...
                print(
                    f"[OTHER TRAY] Resetting NickelQcTrayId {tray.tray_id} to full capacity: {tray.tray_capacity}"
                )
            ...
            return Response(
                {"success": False, "error": f"Failed to update trays: {str(e)}"},
                status=500,
            )
...
class NQValidateTrayIdAPIView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"exists": exists, "valid_for_lot": exists})
...
class NQ_Accepted_form(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            total_stock_data.nq_last_process_date_time = (
                timezone.now()
            )  # Set the last process date/time

            print(
                f"STEP 9: Set nq_last_process_date_time = {total_stock_data.nq_last_process_date_time}"
            )
        ...
        try:
            ...
            if not lot_rejected_comment:
                ...
                return Response(
                    {
                        "success": False,
                        "error": "Lot rejection remarks are required for batch rejection",
                    },
                    status=400,
                )
            ...
            print(
                f"🔍 [NQBatchRejectionAPIView] Searching for JigUnloadAfterTable record..."
            )

            # Method 1: Try to find by direct lot_id match (if lot_id is already unload_lot_id)

            jig_unload_record = JigUnloadAfterTable.objects.filter(
                lot_id=lot_id
            ).first()
            ...
            if jig_unload_record:
                ...
                for record in jig_unload_records:
                    ...
                    if record.combine_lot_ids and isinstance(
                        record.combine_lot_ids, list
                    ):
            ...
            print(
                f"✅ [NQBatchRejectionAPIView] Updated JigUnloadAfterTable record ID {jig_unload_record.id}"
            )
            ...
            print(
                f"✅ [NQBatchRejectionAPIView] Updated {updated_unload_trays_count} JigUnload_TrayId records"
            )
            ...
            print(
                f"✅ [NQBatchRejectionAPIView] Created Nickel_QC_Rejection_ReasonStore record ID {rejection_record.id}"
            )
...
def nickel_get_all_drafts(request):
    ...
    try:
        ...
        if jig_unload_record:
            ...
            for record in JigUnloadAfterTable.objects.filter(
                combine_lot_ids__isnull=False
            ):
        ...
        if not unload_lot_id:
            ...
            return Response(
                {"success": False, "error": "Unload record not found"}, status=404
            )
        ...
        batch_draft = Nickel_QC_Draft_Store.objects.filter(
            lot_id=unload_lot_id, draft_type="batch_rejection"
        ).first()
        ...
        tray_draft = Nickel_QC_Draft_Store.objects.filter(
            lot_id=unload_lot_id, draft_type="tray_rejection"
        ).first()
...
class NQTrayRejectionAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            tray_rejections = data.get(
                "tray_rejections", []
            )  # List of {reason_id, qty, tray_id}
            ...
            print(
                f"🔍 [NQTrayRejectionAPIView] Received tray_rejections: {tray_rejections}"
            )

            if not lot_id or not tray_rejections:

                return Response(
                    {"success": False, "error": "Missing lot_id or tray_rejections"},
                    status=400,
                )
            ...
            print(
                f"🔍 [NQTrayRejectionAPIView] Searching for JigUnloadAfterTable record..."
            )
            ...
            if jig_unload_record:
                ...
                print(
                    f"🔍 [NQTrayRejectionAPIView] Not found by direct match, searching in combine_lot_ids..."
                )
                ...
                for record in jig_unload_records:
                    ...
                    if record.combine_lot_ids and isinstance(
                        ...
                        if lot_id in record.combine_lot_ids:
                            ...
                            print(
                                f"✅ [NQTrayRejectionAPIView] Found in combine_lot_ids"
                            )
            ...
            if not jig_unload_record:
                ...
                print(
                    f"❌ [NQTrayRejectionAPIView] No JigUnloadAfterTable record found for lot_id: {lot_id}"
                )
            ...
            print(
                f"🔍 [NQTrayRejectionAPIView] Available quantity from JigUnloadAfterTable: {available_qty}"
            )
            ...
            print(
                f"🔍 [NQTrayRejectionAPIView] Processing {len(tray_rejections)} individual tray rejections..."
            )
            ...
            for idx, item in enumerate(tray_rejections):
                ...
                try:
                    ...
                    reason_obj = Nickel_QC_Rejection_Table.objects.get(
                        rejection_reason_id=reason_id
                    )

                    print(
                        f"   ✅ Found rejection reason: {reason_obj.rejection_reason}"
                    )
                    ...
                    print(
                        f"   ✅ SAVED rejection record ID {rejection_record.id}: tray_id={tray_id}, qty={qty}, reason={reason_obj.rejection_reason}"
                    )
            ...
            if not saved_rejections:
                ...
                return Response(
                    {"success": False, "error": "No valid rejections were processed"},
                    status=400,
                )
            ...
            if reason_ids_used:
                ...
                reasons = Nickel_QC_Rejection_Table.objects.filter(
                    rejection_reason_id__in=list(reason_ids_used)
                )
                ...
                print(
                    f"✅ [NQTrayRejectionAPIView] Created summary record: total_qty={total_qty}, reasons={len(reasons)}"
                )
            ...
            print(
                f"🔍 [NQTrayRejectionAPIView] Updating TrayId records for {len(unique_tray_ids)} unique trays: {unique_tray_ids}"
            )
            ...
            for tray_id in unique_tray_ids:
                ...
                jig_tray_obj = JigUnload_TrayId.objects.filter(
                    tray_id=tray_id, lot_id=unload_lot_id
                ).first()
                ...
                if jig_tray_obj or tray_obj:
                    ...
                    tray_total_qty = sum(
                        [
                            item["qty"]
                            for item in saved_rejections
                            if item["tray_id"] == tray_id
                        ]
                    )

                    print(
                        f"🔍 [NQTrayRejectionAPIView] Updating tray {tray_id}: total_qty={tray_total_qty}"
                    )
                    ...
                    if tray_obj:
                        ...
                        if is_new_tray:
                            ...
                            main_original_lot_id = (
                                original_lot_ids[0] if original_lot_ids else lot_id
                            )
                            ...
                            print(
                                f"✅ [NQTrayRejectionAPIView] Updated NEW TrayId {tray_id}: lot_id={main_original_lot_id}"
                            )
                            ...
                            print(
                                f"✅ [NQTrayRejectionAPIView] Updated EXISTING TrayId {tray_id}"
                            )
                    ...
                    if nickel_qc_tray_obj:
                        ...
                        nickel_qc_tray_obj.save(
                            update_fields=["tray_quantity", "rejected_tray", "top_tray"]
                        )

                        print(
                            f"✅ [NQTrayRejectionAPIView] Updated NickelQcTrayId for tray {tray_id}: tray_quantity={tray_total_qty}, rejected_tray=True"
                        )
                        ...
                        NickelQcTrayId.objects.create(
                            ...
                            top_tray=False,
                            tray_type=(
                                getattr(tray_obj, "tray_type", None)
                                if tray_obj
                                else None
                            ),
                            tray_capacity=(
                                getattr(tray_obj, "tray_capacity", None)
                                if tray_obj
                                else None
                            ),
                            IP_tray_verified=False,
                        ...
                        print(
                            f"➕ [NQTrayRejectionAPIView] Created new NickelQcTrayId for tray_id={tray_id} (NEW TRAY), tray_quantity={tray_total_qty}"
                        )
                    ...
                    print(
                        f"⚠️ [NQTrayRejectionAPIView] Tray {tray_id} not found in any tray table"
                    )

            print(
                f"✅ [NQTrayRejectionAPIView] Updated {updated_tray_count} tray IDs as rejected"
            )
            ...
            if total_qty >= available_qty:
                ...
                if delink_needed:
                    ...
                    print(
                        "✅ All pieces rejected + delink needed: nq_qc_rejection=True, nq_onhold_picking=True"
                    )
                    ...
                    jig_unload_record.nq_onhold_picking = (
                        False  # Remove from pick table
                    )
                    ...
                    print(
                        "✅ All pieces rejected + no delink: nq_qc_rejection=True, nq_onhold_picking=False"
                    )
                ...
                print(
                    "✅ Partial rejection: nq_qc_few_cases_accptance=True, nq_onhold_picking=True"
                )
            ...
            print(
                f"✅ [NQTrayRejectionAPIView] Updated JigUnloadAfterTable record ID {jig_unload_record.id}"
            )
    ...
    def check_delink_required(self, unload_lot_id, available_qty):
        ...
        try:
            ...
            print(
                f"🔍 [check_delink_required] Checking for unload_lot_id: {unload_lot_id}"
            )
            ...
            if not jig_unload_record:
                ...
                print(
                    f"❌ [check_delink_required] No JigUnloadAfterTable record found for unload_lot_id: {unload_lot_id}"
                )
            ...
            original_distribution = get_nickel_qc_actual_tray_distribution_for_delink(
                unload_lot_id, jig_unload_record
            )

            print(
                f"🔍 [check_delink_required] Original distribution: {original_distribution}"
            )

            if not original_distribution:

                print(
                    f"ℹ️ [check_delink_required] No original distribution - no delink needed"
                )
            ...
            current_distribution = (
                nickel_qc_calculate_distribution_after_rejections_enhanced(
                    unload_lot_id, original_distribution
                )
            )

            print(
                f"🔍 [check_delink_required] Current distribution: {current_distribution}"
            )
            ...
            print(
                f"🔍 [check_delink_required] Final result: delink_needed = {delink_needed}"
            )
...
def nickel_qc_reject_check_tray_id(request):
    ...
    try:
        ...
        already_rejected_in_nickel_qc = Nickel_QC_Rejected_TrayScan.objects.filter(
            lot_id=unload_lot_id, rejected_tray_id=tray_id
        ).exists()
        ...
        return JsonResponse(
            {"exists": False, "error": "System error", "status_message": "System Error"}
        )
...
def nickel_qc_reject_check_tray_id_simple(request):
    ...
    print(
        f"🔍 [Nickel QC Simple] tray_id: {tray_id}, lot_id: {current_lot_id}, qty: {rejection_qty}"
    )
    ...
    try:
        ...
        print(
            f"🔍 [Nickel QC Simple] Overall total_case_qty for unload_lot_id {unload_lot_id}: {overall_qty}"
        )
        ...
        if nickel_qc_tray_obj:
            ...
            print(
                f"🔍 [Nickel QC Simple] Found in NickelQcTrayId for unload_lot_id {unload_lot_id}"
            )
            ...
            if remaining_in_tray > 0:
                ...
                other_trays = NickelQcTrayId.objects.filter(
                    lot_id=unload_lot_id, tray_quantity__gt=0, rejected_tray=False
        ...
        print(
            f"🔍 [Nickel QC Simple] Not found in NickelQcTrayId, checking TrayId for new tray"
        )
        ...
        jig_unload_tray_obj = JigUnload_TrayId.objects.filter(
            tray_id=tray_id, lot_id=unload_lot_id
        ).first()
        ...
        tray_capacity_validation = validate_nickel_qc_tray_capacity_compatibility(
            tray_obj, unload_lot_id
        )
            ...
            return JsonResponse(
                {
                    ...
                    "scanned_tray_capacity": tray_capacity_validation[
                        "scanned_tray_capacity"
                    ],
                    "expected_tray_capacity": tray_capacity_validation[
            ...
            return JsonResponse(
                {
                    ...
                    or tray_capacity_validation["expected_tray_capacity"],
                    "unload_lot_id": unload_lot_id,
                }
            )
...
def validate_nickel_qc_tray_capacity_compatibility(tray_obj, unload_lot_id):
    ...
    try:
        ...
        expected_tray_capacity = get_expected_tray_capacity_for_nickel_qc_lot(
            unload_lot_id
        )

        print(
            f"🔍 [Nickel QC Capacity] Expected tray capacity for unload_lot_id {unload_lot_id}: {expected_tray_capacity}"
        )
        ...
        if not scanned_tray_capacity or not expected_tray_capacity:
            ...
            print(
                f"🔍 [Nickel QC Capacity] Missing capacity info - allowing as fallback"
            )
        ...
        if is_compatible:
            ...
            print(
                f"✅ [Nickel QC Capacity] Compatible: {scanned_tray_capacity} matches {expected_tray_capacity}"
            )
            ...
            print(
                f"❌ [Nickel QC Capacity] Incompatible: {scanned_tray_capacity} ≠ {expected_tray_capacity}"
            )
...
def get_expected_tray_capacity_for_nickel_qc_lot(unload_lot_id):
    ...
    try:
        ...
        if jig_unload_record:
            ...
            if tray_records:
                ...
                if tray_obj and tray_obj.tray_capacity:
                    ...
                    print(
                        f"🔍 [Expected Nickel QC Capacity] Found from JigUnload_TrayId: {tray_obj.tray_capacity}"
                    )
        ...
        existing_tray = NickelQcTrayId.objects.filter(
            lot_id=unload_lot_id, rejected_tray=False, tray_capacity__isnull=False
        ).first()

        if existing_tray and existing_tray.tray_capacity:

            print(
                f"🔍 [Expected Nickel QC Capacity] Found from existing NickelQcTrayId: {existing_tray.tray_capacity}"
            )
        ...
        if jig_unload_record and jig_unload_record.combine_lot_ids:
            ...
            for original_lot_id in jig_unload_record.combine_lot_ids:
                ...
                brass_tray = NickelQcTrayId.objects.filter(
                ...
                if brass_tray and brass_tray.tray_capacity:
                    ...
                    print(
                        f"🔍 [Expected Nickel QC Capacity] Found from original lot NickelQcTrayId: {brass_tray.tray_capacity}"
                    )

                    return brass_tray.tray_capacity

        print(
            f"🔍 [Expected Nickel QC Capacity] Could not determine expected tray capacity for unload_lot_id {unload_lot_id}"
        )
...
def get_nickel_qc_available_quantities_with_session_allocations(
    unload_lot_id, current_session_allocations
):
    """
    ...
    try:
        ...
        saved_rejections = Nickel_QC_Rejected_TrayScan.objects.filter(
            lot_id=unload_lot_id
        ).order_by("id")
        ...
        for rejection in saved_rejections:
            ...
            if tray_id and is_new_tray_by_id(tray_id):
                ...
                available_quantities = nickel_qc_reduce_quantities_optimally(
                    available_quantities, rejected_qty, is_new_tray=True
                )

                print(
                    f"🔍 [Nickel QC Session] NEW tray saved rejection: freed up {rejected_qty} space"
                )
                ...
                available_quantities = nickel_qc_reduce_quantities_optimally(
                    available_quantities, rejected_qty, is_new_tray=False
                )

                print(
                    f"🔍 [Nickel QC Session] EXISTING tray saved rejection: removed tray"
                )
        ...
        for allocation in current_session_allocations:
            ...
            try:
                ...
                if is_new_tray_used:
                    ...
                    available_quantities = nickel_qc_reduce_quantities_optimally(
                        available_quantities, qty, is_new_tray=True
                    )

                    print(
                        f"🔍 [Nickel QC Session] NEW tray session: freed up {qty} space using tray {tray_ids}"
                    )

                else:

                    available_quantities = nickel_qc_reduce_quantities_optimally(
                        available_quantities, qty, is_new_tray=False
                    )
...
def nickel_qc_reduce_quantities_optimally(
    available_quantities, qty_to_reduce, is_new_tray=True
):
    """
    ...
    if is_new_tray:
        ...
        print(
            f"🔍 [nickel_qc_reduce_quantities] NEW tray: freeing up {qty_to_reduce} space"
        )
        ...
        if total_available < qty_to_reduce:
            ...
            print(
                f"🔍 [nickel_qc_reduce_quantities] EXISTING tray: insufficient quantity ({total_available} < {qty_to_reduce})"
            )

            return quantities

        print(
            f"🔍 [nickel_qc_reduce_quantities] EXISTING tray: consuming {qty_to_reduce} pieces"
        )
        ...
        sorted_indices = sorted(
            range(len(temp_quantities)), key=lambda i: temp_quantities[i], reverse=True
        )
        ...
        for i in sorted_indices:
            ...
            if current_qty > 0:
                ...
                print(
                    f"  Consumed {consume_from_this_tray} from tray {i}, new qty: {temp_quantities[i]}"
                )
...
def get_nickel_qc_original_tray_distribution(unload_lot_id):
    ...
    try:
        ...
        print(
            f"🔍 [Nickel QC Distribution] Getting distribution for unload_lot_id: {unload_lot_id}"
        )

        # First try to get from NickelQcTrayId records

        tray_objects = (
            NickelQcTrayId.objects.filter(lot_id=unload_lot_id)
            .exclude(rejected_tray=True)
            .order_by("date")
        )

        print(
            f"🔍 [Nickel QC Distribution] Found {tray_objects.count()} valid NickelQcTrayId objects"
        )
        ...
        if tray_objects.exists():
            ...
            for tray in tray_objects:
                ...
                print(
                    f"🔍 [Nickel QC Distribution] Tray {tray.tray_id}: quantity = {tray_qty}, rejected = {rejected_tray}"
                )
                ...
                if not rejected_tray and tray_qty and tray_qty > 0:
                    ...
                    print(
                        f"🔍 [Nickel QC Distribution] SKIPPED tray {tray.tray_id} - rejected or zero quantity"
                    )

            if quantities:

                print(
                    f"🔍 [Nickel QC Distribution] From NickelQcTrayId objects: {quantities}"
                )
        ...
        if not jig_unload_record:
            ...
            print(
                f"❌ [Nickel QC Distribution] No JigUnloadAfterTable found for unload_lot_id: {unload_lot_id}"
            )
        ...
        print(
            f"🔍 [Nickel QC Distribution] Fallback calculation - total_qty: {total_qty}, tray_capacity: {tray_capacity}"
        )
        ...
        print(
            f"🔍 [Nickel QC Distribution] Calculated: {distribution} (total: {total_qty}, capacity: {tray_capacity})"
        )
...
def get_nickel_qc_tray_capacities_for_lot(unload_lot_id):
    ...
    try:
        ...
        print(
            f"🔍 [get_nickel_qc_tray_capacities] Getting all capacities for unload_lot_id: {unload_lot_id}"
        )
...
def get_nickel_qc_tray_capacity_for_lot(unload_lot_id):
    ...
    try:
        ...
        print(
            f"🔍 [get_nickel_qc_tray_capacity] Getting capacity for unload_lot_id: {unload_lot_id}"
        )
        ...
        if jig_unload_record and jig_unload_record.tray_type:
            ...
            custom_capacity = InprocessInspectionTrayCapacity.objects.filter(
                tray_type__tray_type=jig_unload_record.tray_type, is_active=True
            ).first()

            if custom_capacity:

                print(
                    f"🔍 [get_nickel_qc_tray_capacity] Found custom capacity from InprocessInspectionTrayCapacity: {custom_capacity.custom_capacity}"
                )
        ...
        if tray_objects.exists():
            ...
            if tray_capacity and tray_capacity > 0:
                ...
                print(
                    f"🔍 [get_nickel_qc_tray_capacity] Found from NickelQcTrayId: {tray_capacity}"
                )
            ...
            for tray in tray_objects:
                ...
                if capacity and capacity > 0:
                    ...
                    print(
                        f"🔍 [get_nickel_qc_tray_capacity] Found valid capacity: {capacity}"
                    )
        ...
        if jig_tray_objects.exists():
            ...
            for jig_tray in jig_tray_objects:
                ...
                if tray_obj and tray_obj.tray_capacity:
                    ...
                    print(
                        f"🔍 [get_nickel_qc_tray_capacity] Found from JigUnload_TrayId: {tray_obj.tray_capacity}"
                    )
        ...
        if jig_unload_record and jig_unload_record.combine_lot_ids:
            ...
            for original_lot_id in jig_unload_record.combine_lot_ids:
                ...
                if brass_tray and brass_tray.tray_capacity:
                    ...
                    print(
                        f"🔍 [get_nickel_qc_tray_capacity] Found from original lot: {brass_tray.tray_capacity}"
                    )
...
def nickel_qc_calculate_distribution_after_rejections_enhanced(
    unload_lot_id, original_distribution
):
    """
    ...
    rejections = Nickel_QC_Rejected_TrayScan.objects.filter(
        lot_id=unload_lot_id
    ).order_by("id")

    print(
        f"🔧 [Nickel QC Enhanced Distribution] Starting with: {original_distribution}"
    )

    print(
        f"🔧 [Nickel QC Enhanced Distribution] Processing {rejections.count()} rejections for unload_lot_id {unload_lot_id}"
    )

    for idx, rejection in enumerate(rejections):

        rejected_qty = (
            int(rejection.rejected_tray_quantity)
            if rejection.rejected_tray_quantity
            else 0
        )
        ...
        reason = (
            rejection.rejection_reason.rejection_reason
        ...
        if not tray_id or tray_id.strip() == "":
            ...
            current_distribution = nickel_qc_consume_shortage_from_distribution(
        ...
        if is_new_tray:
            ...
            print(
                f"   - NEW tray used - freeing up {rejected_qty} space in existing trays"
            )

            current_distribution = nickel_qc_free_up_space_optimally(
                current_distribution, rejected_qty
            )
            ...
            current_distribution = nickel_qc_remove_rejected_tray_from_distribution(
                current_distribution, rejected_qty
            )
    ...
    print(
        f"🔧 [Nickel QC Enhanced Distribution] FINAL distribution: {current_distribution}"
    )
...
def nickel_qc_get_delink_tray_data(request):
    ...
    try:
        ...
        if not jig_unload_record:
            ...
            return Response(
                {"success": False, "error": "Unload record not found"}, status=400
            )

        print(
            f"🔍 [nickel_qc_get_delink_tray_data] Using unload_lot_id: {unload_lot_id}"
        )
        ...
        print(
            f"🔍 [nickel_qc_get_delink_tray_data] Original distribution: {original_distribution}"
        )
        ...
        if not rejections.exists():
            ...
            print(
                "ℹ️ [nickel_qc_get_delink_tray_data] No rejections found - no delink needed"
            )
        ...
        print(
            f"🔍 [nickel_qc_get_delink_tray_data] Found {rejections.count()} rejections"
        )
        ...
        print(
            f"🔍 [nickel_qc_get_delink_tray_data] Current distribution after rejections: {current_distribution}"
        )
        ...
        rejection_tray_map = (
            {}
        )  # {tray_id: {is_new, total_qty_rejected, tray_capacity, rejection_count}}
        ...
        for rejection in rejections:
            ...
            if tray_obj:
                ...
                if tray_id in rejection_tray_map:
                    ...
                    print(
                        f"📊 [Delink Analysis] Tray {tray_id}: is_new={is_new}, qty_rejected={qty_rejected} (cumulative: {rejection_tray_map[tray_id]['total_qty_rejected']}/{tray_capacity})"
                    )
                    ...
                    print(
                        f"📊 [Delink Analysis] Tray {tray_id}: is_new={is_new}, qty_rejected={qty_rejected}, capacity={tray_capacity}"
                    )
        ...
        print(
            "📊 [Delink Analysis] Checking which NEW trays became completely empty..."
        )
        ...
        for tray_id, info in rejection_tray_map.items():
            ...
            if info["is_new"] and info["tray_capacity"] > 0:
                ...
                if info["total_qty_rejected"] >= info["tray_capacity"]:
                    ...
                    print(
                        f"✅ [Delink Analysis] NEW tray {tray_id}: {info['total_qty_rejected']}/{info['tray_capacity']} → Fully consumed, NEEDS DELINK"
                    )
                    ...
                    print(
                        f"❌ [Delink Analysis] NEW tray {tray_id}: {info['total_qty_rejected']}/{info['tray_capacity']} → Partially used, NO delink"
                    )

            elif not info["is_new"]:

                print(
                    f"❌ [Delink Analysis] EXISTING tray {tray_id}: Not eligible for delink (existing tray)"
                )

        # 🔥 FIXED: Match empty positions with NEW trays that need delink

        print(
            "📊 [Delink Analysis] Matching empty positions to delink-eligible trays..."
        )
        ...
        for i, current_qty in enumerate(current_distribution):
            ...
            if current_qty == 0:
                ...
                original_capacity = (
                    original_distribution[i] if i < len(original_distribution) else 0
                )

                if original_capacity > 0:

                    print(
                        f"📊 [Delink Analysis] Position {i+1}: Empty (was {original_capacity})"
                    )
                    ...
                    if len(new_trays_needing_delink) > len(empty_tray_positions):
                        ...
                        print(
                            f"✅ [Delink Analysis] Position {i+1}: Matched to NEW tray {tray_id} → NEEDS DELINK"
                        )

                    else:

                        print(
                            f"❌ [Delink Analysis] Position {i+1}: Empty but no more NEW trays need delink"
                        )

        print(
            f"🔍 [nickel_qc_get_delink_tray_data] Empty tray positions needing delink: {empty_tray_positions}"
        )

        print(
            f"🔍 [nickel_qc_get_delink_tray_data] Total NEW trays needing delink: {len(delink_trays)}"
        )

        if len(delink_trays) == 0:

            print(
                "ℹ️ [nickel_qc_get_delink_tray_data] No NEW trays became completely empty - no delink needed"
            )
...
def get_nickel_qc_actual_tray_distribution_for_delink(unload_lot_id, jig_unload_record):
    ...
    try:
        ...
        print(
            f"🔍 [get_nickel_qc_actual_tray_distribution_for_delink] Getting distribution for unload_lot_id: {unload_lot_id}"
        )
...
def nickel_qc_consume_shortage_from_distribution(distribution, shortage_qty):
    ...
    for i in sorted_indices:
        ...
        if current_qty >= remaining_shortage:
            ...
            print(
                f"   Consumed {remaining_shortage} from tray {i}, remaining: {result[i]}"
            )
    ...
    if remaining_shortage > 0:
        ...
        print(
            f"   ⚠️ WARNING: Could not consume all shortage qty, remaining: {remaining_shortage}"
        )
...
def nickel_qc_free_up_space_optimally(distribution, qty_to_free):
    ...
    print(
        f"   🔧 [Nickel QC Free Up Space] Input: {distribution}, qty_to_free: {qty_to_free}"
    )
    ...
    print(
        f"   🔧 [Nickel QC Free Up Space] Processing order (smallest first): {sorted_indices}"
    )
    ...
    for i in sorted_indices:
        ...
        if current_qty >= remaining:
            ...
            print(
                f"   🔧 [Nickel QC Free Up Space] Freed {remaining} from tray {i+1}, new qty: {result[i]}"
            )
            ...
            print(
                f"   🔧 [Nickel QC Free Up Space] Freed entire tray {i+1}: {current_qty} -> 0"
            )
...
def nickel_delink_check_tray_id(request):
    ...
    try:
        ...
        if not tray_obj:
            ...
            tray_exists_elsewhere = NickelQcTrayId.objects.filter(
                tray_id=tray_id
            ).exists()
...
def nickel_get_accepted_tray_scan_data(request):
    ...
    try:
        ...
        total_rejection_qty = (
            reason_store.total_rejection_quantity if reason_store else 0
        )
        ...
        if nq_physical_qty <= 0:
            ...
            return Response(
                {"success": False, "error": "No nickel physical quantity available"},
                status=400,
            )
        ...
        print(
            f"🔍 [nickel_get_accepted_tray_scan_data] nq_physical_qty = {nq_physical_qty}"
        )

        print(
            f"🔍 [nickel_get_accepted_tray_scan_data] total_rejection_qty = {total_rejection_qty}"
        )

        print(
            f"🔍 [nickel_get_accepted_tray_scan_data] available_qty = {available_qty}"
        )
        ...
        if is_delink_only_case:
            ...
            print(
                f"🚨 [nickel_get_accepted_tray_scan_data] Delink-only case detected: all pieces rejected"
            )
        ...
        if available_qty <= 0:
            ...
            return Response(
                {
                    "success": False,
                    "error": "No available quantity for acceptance after rejections",
                },
                status=400,
            )
        ...
        print(
            f"📊 [nickel_get_accepted_tray_scan_data] Tray calculation: {available_qty} qty = {full_trays} full trays + {top_tray_qty} top tray"
        )

        # Check for existing draft data

        has_draft = Nickel_QC_TopTray_Draft_Store.objects.filter(
            lot_id=unload_lot_id
        ).exists()
        ...
        if has_draft:
            ...
            draft_record = Nickel_QC_TopTray_Draft_Store.objects.filter(
                lot_id=unload_lot_id
            ).first()
...
def nickel_save_single_top_tray_scan(request):
    ...
    try:
        ...
        if tray_id:
            ...
            delink_tray_ids = [
                delink["tray_id"] for delink in delink_trays if delink.get("tray_id")
            ]
        ...
        if not draft_save and delink_trays:
            ...
            for delink in delink_trays:
                ...
                if delink_tray_id:
                    ...
                    delink_tray_obj = NickelQcTrayId.objects.filter(
                        tray_id=delink_tray_id, lot_id=unload_lot_id
                    ).first()
        ...
        if not draft_save:
            ...
            if tray_id:
                ...
                if top_tray_obj:
                    ...
                    print(
                        f"✅ [nickel_save_single_top_tray_scan] Updated top tray: {tray_id}"
                    )

                # Update all other trays (except rejected and top tray) to have tray_quantity = tray_capacity

                all_trays_in_lot = NickelQcTrayId.objects.filter(
                    lot_id=unload_lot_id, rejected_tray=False
                )
                ...
                for tray in all_trays_in_lot:
                    ...
                    print(
                        f"   Updated NickelQcTrayId tray {tray.tray_id}: qty {old_qty}→{tray.tray_capacity}, top_tray=False"
                    )
            ...
            for delink in delink_trays:
                ...
                if delink_tray_id:
                    ...
                    nickel_delink_tray_obj = NickelQcTrayId.objects.filter(
                        tray_id=delink_tray_id, lot_id=unload_lot_id
                    ).first()
        ...
        if draft_save:
            ...
            draft_obj, created = Nickel_QC_TopTray_Draft_Store.objects.update_or_create(
                ...
                defaults={
                    ...
                    "delink_tray_ids": [
                        d["tray_id"] for d in delink_trays if d.get("tray_id")
                    ],
                    "delink_trays_data": {
                        "positions": [
                            {
...
def nickel_get_top_tray_scan_draft(request):
    ...
    try:
        ...
        draft_obj = Nickel_QC_TopTray_Draft_Store.objects.filter(
            lot_id=unload_lot_id
        ).first()
        ...
        if draft_obj:
            ...
            return Response(
                {
                    ...
                    "draft_data": {
                        ...
                        "delink_trays": (
                            draft_obj.delink_trays_data.get("positions", [])
                            if draft_obj.delink_trays_data
                            else []
                        ),
                    },
                    "unload_lot_id": unload_lot_id,
            ...
            return Response(
                {"success": True, "has_draft": False, "unload_lot_id": unload_lot_id}
            )
...
def nickel_view_tray_list(request):
    ...
    try:
        ...
        if batch_rejection and total_rejection_qty > 0 and tray_capacity > 0:
            ...
            tray_ids = list(
                NickelQcTrayId.objects.filter(lot_id=unload_lot_id).values_list(
                    "tray_id", flat=True
                )
            )
            ...
            trays = Nickel_Qc_Accepted_TrayID_Store.objects.filter(
                lot_id=unload_lot_id
            ).order_by("id")
...
class NickelTrayValidateAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            lot_id_input = str(
                data.get("batch_id", "") or data.get("lot_id", "")
            ).strip()
            ...
            print(
                f"[NICKEL DEBUG] Extracted lot_id: '{lot_id_input}' (length: {len(lot_id_input)})"
            )

            print(
                f"[NICKEL DEBUG] Extracted tray_id: '{tray_id}' (length: {len(tray_id)})"
            )

            if not lot_id_input or not tray_id:

                return JsonResponse(
                    {"success": False, "error": "Both lot_id and tray_id are required"},
                    status=400,
                )
            ...
            print(
                f"[NICKEL DEBUG] Finding unload_lot_id for input lot_id: '{lot_id_input}'"
            )
            ...
            if jig_unload_record:
                ...
                print(
                    f"[NICKEL DEBUG] Not found by direct match, searching in combine_lot_ids..."
                )
                ...
                for record in jig_unload_records:
                    ...
                    if record.combine_lot_ids and isinstance(
                        ...
                        if lot_id_input in record.combine_lot_ids:
                            ...
                            print(
                                f"[NICKEL DEBUG] original_lot_ids: {original_lot_ids}"
                            )
            ...
            if not unload_lot_id:
                ...
                print(
                    f"[NICKEL DEBUG] No JigUnloadAfterTable record found for lot_id: '{lot_id_input}'"
                )
            ...
            print(
                f"[NICKEL DEBUG] Checking if tray '{tray_id}' exists in NickelQcTrayId for unload_lot_id: '{unload_lot_id}'"
            )

            tray_exists = NickelQcTrayId.objects.filter(
                lot_id=unload_lot_id, tray_id=tray_id  # Use unload_lot_id
            ).exists()
            ...
            print(
                f"[NICKEL DEBUG] All trays in NickelQcTrayId for unload_lot_id '{unload_lot_id}': {list(all_nickel_trays)}"
            )
            ...
            if tray_anywhere_nickel.exists():
                ...
                tray_lot_ids_nickel = list(
                    tray_anywhere_nickel.values_list("lot_id", flat=True)
                )

                print(
                    f"[NICKEL DEBUG] Tray '{tray_id}' found in NickelQcTrayId for lot_ids: {tray_lot_ids_nickel}"
                )
            ...
            jig_unload_tray_exists = JigUnload_TrayId.objects.filter(
                lot_id=unload_lot_id, tray_id=tray_id
            ).exists()

            print(
                f"[NICKEL DEBUG] Tray exists in JigUnload_TrayId: {jig_unload_tray_exists}"
            )
            ...
            if tray_exists:
                ...
                if nickel_tray_obj:
                    ...
                    tray_info = {
                        ...
                        "rejected_tray": getattr(
                            nickel_tray_obj, "rejected_tray", False
                        ),
                        "top_tray": getattr(nickel_tray_obj, "top_tray", False),
                        "delink_tray": getattr(nickel_tray_obj, "delink_tray", False),
                        "IP_tray_verified": getattr(
                            nickel_tray_obj, "IP_tray_verified", False
                        ),
...
def nickel_check_accepted_tray_draft(request):
    ...
    try:
        ...
        has_draft = Nickel_Qc_Accepted_TrayID_Store.objects.filter(
            lot_id=unload_lot_id, is_draft=True  # Use unload_lot_id
        ).exists()

        return Response(
            {"success": True, "has_draft": has_draft, "unload_lot_id": unload_lot_id}
        )
...
def nickel_save_accepted_tray_scan(request):
    ...
    try:
        ...
        if not lot_id or not rows:
            ...
            return Response(
                {"success": False, "error": "Missing lot_id or rows"}, status=400
            )
        ...
        for idx, row in enumerate(rows):
            ...
            if (
                not tray_id
                or not NickelQcTrayId.objects.filter(
                    tray_id=tray_id, lot_id=unload_lot_id
                ).exists()
            ):
        ...
        return Response(
            {"success": True, "message": "Nickel accepted tray scan saved."}
        )
...
def nickel_check_tray_id(request):
    ...
    if not unload_lot_id:
        ...
        return JsonResponse({"exists": False, "error": "Unload record not found"})
    ...
    return JsonResponse(
        {
            ...
            "rejected_in_nickel_qc": exists
            and getattr(tray_obj, "rejected_tray", False),
            "unload_lot_id": unload_lot_id,
        }
    )
...
class NQCompletedView(APIView):
    ...
    def get(self, request):
        ...
        queryset = (
            ...
            .filter(
                nq_last_process_date_time__range=(
                    from_datetime,
                    to_datetime,
                )  # Filter by NQ process date, not creation date
            )
            .order_by("-nq_last_process_date_time", "-lot_id")
        )

        print(
            f"📊 Found {queryset.count()} nickel records in date range {from_date} to {to_date}"
        )

        print(
            "All lot_ids in completed queryset:",
            list(queryset.values_list("lot_id", flat=True)),
        )
        ...
        for jig_unload_obj in page_obj.object_list:
            ...
            data = {
                ...
                "tray_capacity": apply_tray_capacity_correction(
                    jig_unload_obj.tray_capacity or 0, None, jig_unload_obj.lot_id
                ),
            ...
            if jig_unload_obj.plating_stk_no:
                ...
                if len(plating_stk_no) >= 4:
                    ...
                    print(
                        f"🎯 NQ Completed View - Extracted model_no: {model_no_prefix} from plating_stk_no: {plating_stk_no}"
                    )
                    ...
                    try:
                        ...
                        if model_master:
                            ...
                            print(
                                f"✅ NQ Completed View - Found ModelMaster for images: {model_master.model_no}"
                            )
                            ...
                            for img in model_master.images.all():
                                ...
                                if img.master_image:
                                    ...
                                    print(
                                        f"📸 NQ Completed View - Added image from ModelMaster: {img.master_image.url}"
                                    )

                        else:

                            print(
                                f"⚠️ NQ Completed View - No ModelMaster found for model_no: {model_no_prefix}"
                            )
            ...
            if not images and data["combine_lot_ids"]:
                ...
                print(
                    "🔄 NQ Completed View - No ModelMaster images, trying combine_lot_ids fallback"
                )
                        ...
                        if batch_obj.model_stock_no:
                            ...
                            for img in batch_obj.model_stock_no.images.all():
                                ...
                                if img.master_image:
                                    ...
                                    print(
                                        f"📸 NQ Completed View - Added image from TotalStockModel: {img.master_image.url}"
                                    )
            ...
            print(
                f"📸 NQ Completed View - Final images for lot {jig_unload_obj.lot_id}: {len(images)} images"
            )
        ...
        for data in master_data:
            ...
            if total_IP_accpeted_quantity and total_IP_accpeted_quantity > 0:
                ...
                if jig_unload_obj and total_rejection_qty > 0:
                    ...
                    print(
                        f"Calculated accepted qty for {lot_id}: {jig_unload_obj.total_case_qty} - {total_rejection_qty} = {data['display_accepted_qty']}"
                    )
...
class BrassGetShortageRejectionsView(APIView):
    ...
    def get(self, request):
        ...
        shortage_rejections = IP_Rejected_TrayScan.objects.filter(
            lot_id=lot_id, rejected_tray_id__isnull=True
        ).union(IP_Rejected_TrayScan.objects.filter(lot_id=lot_id, rejected_tray_id=""))
        ...
        return JsonResponse({"success": True, "shortage_rejections": shortage_data})
...
def nickel_get_draft_data(request):
    ...
    try:
        ...
        draft_obj = Nickel_QC_Draft_Store.objects.filter(
            lot_id=unload_lot_id, draft_type=draft_type  # Use unload_lot_id
        ).first()
    ...
    def post(self, request):
        ...
        try:
            ...
            if not lot_id or not draft_type:
                ...
                return Response(
                    {"success": False, "error": "Missing lot_id or draft_type"},
                    status=400,
                )
...
def _get_expected_tray_prefix_for_lot(lot_id):
    ...
    try:
        ...
        # Prefer tray_type stored directly on JigUnloadAfterTable

        if jig_record.tray_type:

            return jig_record.tray_type.upper()

        # Fallback: look up via plating_stk_no -> ModelMaster -> TrayType

        if jig_record.plating_stk_no:

            mm = (
                ModelMaster.objects.filter(plating_stk_no=jig_record.plating_stk_no)
                .select_related("tray_type")
                .first()
            )
...
def _is_tray_new_or_delinked(tray_id):
    ...
    if not master:
        ...
        return True  # Not in master table = new tray
...
def _is_tray_occupied_by_other_lot(tray_id, lot_id):
    ...
    if (
        master
        and master.lot_id
        and master.lot_id.strip()
        and master.lot_id != lot_id
        and not master.delink_tray
    ):

        return True, master.lot_id

    # Check NickelQcTrayId

    nq = (
        NickelQcTrayId.objects.filter(tray_id=tray_id)
        .exclude(lot_id=lot_id)
        .filter(delink_tray=False)
        .first()
    )

    if nq and nq.lot_id:

        return True, nq.lot_id
    ...
    def post(self, request):
        ...
        try:
            ...
            if not tray_id:
                ...
                return JsonResponse(
                    {"success": False, "error": "Missing tray_id"}, status=400
                )

            print(
                f"[PickTrayValidate_Complete_APIView] Checking tray_id '{tray_id}' for lot_id '{lot_id}'"
            )
            ...
            if expected_prefix and not is_new_or_delinked:
                ...
                if tray_prefix and tray_prefix != expected_prefix:
                    ...
                    print(
                        f"❌ [PickTrayValidate] Prefix mismatch: tray '{tray_id}' prefix '{tray_prefix}' vs expected '{expected_prefix}'"
                    )
            ...
            if not is_new_or_delinked:
                ...
                if occupied:
                    ...
                    print(
                        f"❌ [PickTrayValidate] Tray '{tray_id}' occupied by lot '{occupied_lot}'"
                    )
            ...
            tray = NickelQcTrayId.objects.filter(lot_id=lot_id, tray_id=tray_id).first()
            ...
            if tray is None:
                ...
                print(
                    f"⚠️ Tray not found in NickelQcTrayId, checking JigUnload_TrayId..."
                )
            ...
            if exists:
                ...
                tray_info = {
                    "tray_id": tray.tray_id,
                    "tray_quantity": getattr(
                        tray, "tray_quantity", getattr(tray, "tray_qty", 0)
                    ),
                    "lot_id": tray.lot_id,
                }
...
class PickTrayIdList_Complete_APIView(APIView):
    ...
    def get(self, request):
        ...
        print(
            f"🔍 [PickTrayIdList_Complete_APIView] Received lot_id: {lot_id}, include_drafts: {include_drafts}"
        )
        ...
        if trays.count() == 0:
            ...
            print(
                f"⚠️ No NickelQcTrayId records found for lot_id: {lot_id}, checking JigUnload_TrayId..."
            )
        ...
        if trays.count() > 0:
            ...
            for index, tray in enumerate(trays, 1):
                ...
                print(
                    f"    - Tray {index}: {tray.tray_id} (qty: {tray_data['tray_quantity']}) top={tray_data['top_tray']} delink={tray_data['delink_tray']} rejected={tray_data['rejected_tray']}"
                )
            ...
            print(
                f"⚠️ No JigUnload_TrayId for lot_id: {lot_id}, trying upstream tray tables..."
            )
            ...
            if upstream_data:
                ...
                print(
                    f"❌ [PickTrayIdList_Complete_APIView] No tray records found for lot_id: {lot_id}"
                )
        ...
        if include_drafts:
            ...
            if tray_draft and tray_draft.draft_data:
                ...
                print(
                    f"📝 [PickTrayIdList_Complete_APIView] Found {len(draft_tray_rejections)} draft rejections to merge for {unload_lot_id}"
                )
        ...
        return JsonResponse(
            {"success": True, "trays": data, "tray_source": tray_source}
        )
...
class NickelQcTrayIdList_Complete_APIView(APIView):
    ...
    def get(self, request):
        ...
        print(
            f"🔍 [NickelQcTrayIdList_Complete_APIView] Received lot_id: {lot_id}, include_drafts: {include_drafts}"
        )
        ...
        if trays.count() == 0:
            ...
            print(
                f"⚠️ [NickelQcTrayIdList_Complete] No NickelQcTrayId records, checking JigUnload_TrayId..."
            )
        ...
        if include_drafts:
            ...
            if tray_draft and tray_draft.draft_data:
                ...
                print(
                    f"📝 [NickelQcTrayIdList_Complete_APIView] Found {len(draft_tray_rejections)} draft rejections to merge for {unload_lot_id}"
                )
...
def autosave_nickel_qc(request):
    ...
    try:
        ...
        print(
            f"🔄 BACKEND: Received lot_id: {lot_id}, auto_save_data: {auto_save_data}"
        )
        ...
        autosave_obj = Nickel_QC_AutoSave.objects.create(
            lot_id=lot_id, user=request.user, auto_save_data=auto_save_data
        )
...
def load_autosave_nickel_qc(request, lot_id):
    ...
    try:
        ...
        if autosave:
            ...
            return JsonResponse(
                {"success": True, "auto_save_data": autosave.auto_save_data}
            )

        else:

            return JsonResponse({"success": True, "auto_save_data": None})
...
@require_GET
def nq_get_lot_id_for_tray(request):
    """Get lot_id for a scanned tray_id for Nickel Inspection barcode scanner."""
    ...
    try:
        ...
        if lot_id:
            ...
            return JsonResponse(
                {"success": False, "error": "Tray not found", "tray_id": tray_id}
            )

    except Exception as e:

        return JsonResponse(
            {"success": False, "error": str(e), "tray_id": tray_id}, status=500
        )
...
def _nq_resolve_unload_record(lot_id):
    """Resolve JigUnloadAfterTable record from lot_id."""

    record = JigUnloadAfterTable.objects.filter(lot_id=lot_id).first()

    if record:

        return record, str(record.lot_id)

    for rec in JigUnloadAfterTable.objects.filter(
        combine_lot_ids__isnull=False
    ).exclude(combine_lot_ids__exact=[]):

        if (
            rec.combine_lot_ids
            and isinstance(rec.combine_lot_ids, list)
            and lot_id in rec.combine_lot_ids
        ):

            return rec, str(rec.lot_id)

    return None, None


def _nq_resolve_lot_trays(unload_lot_id):
    """Resolve tray data for a Nickel QC lot. Returns (tray_list, total_qty).



    Falls back to JigUnload_TrayId when NickelQcTrayId has no records (first-time rejection).



    """

    # Primary: NickelQcTrayId

    trays = NickelQcTrayId.objects.filter(
        lot_id=unload_lot_id, rejected_tray=False, delink_tray=False
    ...
    # Fallback: JigUnload_TrayId (uses tray_qty field, not tray_quantity)

    if not tray_data:

        jig_trays = JigUnload_TrayId.objects.filter(
            lot_id=unload_lot_id, rejected_tray=False, delink_tray=False
        ).order_by("-top_tray", "tray_id")
    ...
    # Last-resort fallback: look up real tray IDs from upstream tables (no placeholders)

    if not tray_data:

        upstream_data, _ = get_upstream_tray_distribution(unload_lot_id)

        if upstream_data:

            for t in upstream_data:
                ...
                if not t.get("delink_tray") and not t.get("rejected_tray"):
                    ...
                    tray_data.append(
                        {
                            "tray_id": t["tray_id"],
                            "qty": t.get("tray_quantity", 0) or 0,
                            "is_top": bool(t.get("top_tray", False)),
                            "is_rejected": bool(t.get("rejected_tray", False)),
                            "is_delinked": bool(t.get("delink_tray", False)),
                        }
                    )

    total_qty = sum(td["qty"] for td in tray_data)

    return tray_data, total_qty


def _nq_get_tray_capacity(unload_lot_id):
    """Get tray capacity for a Nickel QC lot."""

    nq_tray = NickelQcTrayId.objects.filter(
        lot_id=unload_lot_id, tray_capacity__isnull=False
    ).first()

    if nq_tray and nq_tray.tray_capacity:

        return nq_tray.tray_capacity

    try:

        cap = get_nickel_qc_tray_capacity_for_lot(unload_lot_id)
...
def _nq_compute_reuse_trays(trays, reject_qty):
    """Nickel QC: No tray reuse. Rejection always uses new JB/NB blue trays."""

    return []


def _nq_compute_accept_slots_from_trays(original_trays, rejected_qty):
    """Derive accept slots directly from actual original trays, subtracting rejected qty from top.



    Returns (accept_slots, delinked_trays) where delinked_trays are NR trays fully consumed by rejection.
    ...
    sorted_trays = sorted(
        original_trays, key=lambda x: (not x.get("is_top", False), x.get("tray_id", ""))
    )

    accept_slots = []

    delinked_trays = []  # NR trays fully consumed by rejection — must be delinked
    ...
    for tray in sorted_trays:
        ...
        if remaining_reject >= tray_qty:
            ...
            remaining_reject -= tray_qty  # fully rejected — add to auto-delink list

            delinked_trays.append({"tray_id": tray["tray_id"], "qty": tray_qty})

        elif remaining_reject > 0:

            accept_qty = tray_qty - remaining_reject

            remaining_reject = 0

            accept_slots.append(
                {"tray_id": tray["tray_id"], "qty": accept_qty, "is_top": True}
            )

        else:

            is_top = bool(tray.get("is_top", False)) and len(accept_slots) == 0

            accept_slots.append(
                {"tray_id": tray["tray_id"], "qty": tray_qty, "is_top": is_top}
            )

    return accept_slots, delinked_trays


def _nq_compute_reject_slots(rejected_qty, blue_tray_capacity=12):
    ...
    rem = rejected_qty % blue_tray_capacity

    slots = []

    if rem > 0:

        slots.append({"qty": rem, "is_top": True, "tray_id": None})

    for _ in range(full):

        slots.append({"qty": blue_tray_capacity, "is_top": False, "tray_id": None})
...
@api_view(["POST"])
@permission_classes([IsAuthenticated])
def nickel_qc_action(request):
    """



    UNIFIED Nickel QC Reject API — mirrors Brass QC brass_qc_action.



    Actions: GET_REASONS, GET_TRAYS, ALLOCATE, VALIDATE_TRAY,



             FULL_REJECT, PROCESS, SAVE_DRAFT, GET_DRAFT
    ...
    lot_id = (
        request.data.get("lot_id", "").strip() if request.data.get("lot_id") else ""
    )

    if action == "GET_REASONS":

        reasons = Nickel_QC_Rejection_Table.objects.all().order_by(
            "rejection_reason_id"
        )
        ...
        data = [
            {
                "id": r.id,
                "reason_id": r.rejection_reason_id,
    ...
    if action == "GET_TRAYS":
        ...
        jig_rec, unload_lot_id = _nq_resolve_unload_record(lot_id)

        if not jig_rec:

            return JsonResponse(
                {"error": "Lot not found in unload records"}, status=404
            )

        tray_data, total_qty = _nq_resolve_lot_trays(unload_lot_id)

        tray_capacity = _nq_get_tray_capacity(unload_lot_id)
        ...
        logger.info(
            f"[NQ_ACTION:GET_TRAYS] lot_id={lot_id}, unload={unload_lot_id}, trays={len(active_trays)}, total_qty={total_qty}"
        )
    ...
    if action == "ALLOCATE":
        ...
        if not lot_id:
            ...
            return JsonResponse(
                {"success": False, "error": "lot_id is required"}, status=400
            )
        ...
        if rejected_qty < 0 or rejected_qty > total_qty:
            ...
            return JsonResponse(
                {"success": False, "error": "Invalid rejected_qty"}, status=400
            )

        accepted_qty = total_qty - rejected_qty

        # Nickel QC: accept slots derived from ACTUAL original trays (real IDs + real qtys)

        # delinked_trays = NR trays fully consumed by rejection (auto-delinked, shown read-only in UI)

        accept_slots, delinked_trays = _nq_compute_accept_slots_from_trays(
            active_trays, rejected_qty
        )

        # Reject slots use JB blue tray capacity = 12 (most restrictive; NB=16 validated on scan)

        reject_slots = (
            _nq_compute_reject_slots(rejected_qty) if rejected_qty > 0 else []
        )

        # Build delink_slots: pre-filled with auto-delinked NR tray IDs (read-only in UI)

        delink_slots = [
            {"tray_id": t["tray_id"], "qty": t["qty"], "auto": True}
    ...
    if action == "VALIDATE_TRAY":

        tray_id = request.data.get("tray_id", "").strip()

        slot_type = request.data.get("slot_type", "accept").strip()

        if not tray_id:

            return JsonResponse(
                {"valid": False, "error": "tray_id is required"}, status=400
            )
        ...
        if slot_type == "reject":
            ...
            if not (tray_id.startswith("JB-") or tray_id.startswith("NB-")):
                ...
                return JsonResponse(
                    {
                        "valid": False,
                        "error": "Only JB (Jumbo Blue) or NB (Normal Blue) trays allowed for rejection",
                    }
                )
            ...
            if slot_qty > 0 and slot_qty > max_cap:
                ...
                return JsonResponse(
                    {
                        "valid": False,
                        "error": f"Qty {slot_qty} exceeds {tray_id[:2]} tray max capacity ({max_cap})",
                    }
                )
            ...
            if not tray:
                ...
                return JsonResponse(
                    {"valid": False, "error": "Tray ID not found in tray master"}
                )

            # Check tray is free — not currently assigned to an active NickelQcTrayId lot

            active_use = (
                NickelQcTrayId.objects.filter(tray_id=tray_id, rejected_tray=False)
                .exclude(lot_id=lot_id)
                .first()
            )
            ...
            if active_use:
                ...
                return JsonResponse(
                    {
                        "valid": False,
                        "error": f"Tray {tray_id} is currently in use by lot {active_use.lot_id}",
                    }
                )

            return JsonResponse({"valid": True, "max_capacity": max_cap})
            ...
            if not tray:
                ...
                if not nq_tray and not ju_tray:

                    return JsonResponse(
                        {"valid": False, "error": "Tray ID not found in system"}
                    )
    ...
    if action == "SAVE_DRAFT":
        ...
        if not draft_payload:
            ...
            return JsonResponse(
                {"success": False, "error": "draft_data is required"}, status=400
            )
        ...
        draft, created = Nickel_QC_Draft_Store.objects.update_or_create(
            lot_id=unload_lot_id,
            draft_type="rejection_draft",
            defaults={
                "batch_id": (
                    str(jig_rec.unload_lot_id)
                    if hasattr(jig_rec, "unload_lot_id")
                    else ""
                ),
                "user": request.user,
        ...
        jig_rec.nq_draft = True

        jig_rec.nq_onhold_picking = True

        jig_rec.save(update_fields=["nq_draft", "nq_onhold_picking"])

        logger.info(f"[NQ_DRAFT] Saved for lot_id={lot_id}, unload={unload_lot_id}")
    ...
    if action == "GET_DRAFT":
        ...
        if not jig_rec:
            ...
            return JsonResponse(
                {"success": True, "has_draft": False, "draft_data": None}
            )

        draft = Nickel_QC_Draft_Store.objects.filter(
            lot_id=unload_lot_id, draft_type="rejection_draft"
        ).first()
        ...
        return JsonResponse(
            {"success": True, "has_draft": True, "draft_data": draft.draft_data}
        )

    if action == "FULL_REJECT":
        ...
        qty = jig_rec.total_case_qty or 0
        ...
        logger.info(
            f"[NQ_ACTION:FULL_REJECT] lot_id={lot_id}, unload={unload_lot_id}, qty={qty}"
        )

        return JsonResponse(
            {
                "success": True,
                "message": f"Full lot rejected ({qty} pcs). Moved to Nickel Audit.",
                "lot_id": lot_id,
            }
        )

    if action == "PROCESS":
        tray_actions = request.data.get("tray_actions", [])
        rejection_reasons = request.data.get("rejection_reasons", [])
        remarks = request.data.get("remarks", "").strip()
        if not lot_id:
            return JsonResponse(
        ...
        if not tray_actions:
            return JsonResponse(
                {"success": False, "error": "tray_actions required"}, status=400
            )
        jig_rec, unload_lot_id = _nq_resolve_unload_record(lot_id)
        if not jig_rec:
            return JsonResponse(
        ...
        if accepted_qty < 0:
            return JsonResponse(
                {
                    "success": False,
                    "error": f"Invalid quantities: Accepted ({accepted_qty}) cannot be negative",
                },
                status=400,
            )
        ...
        for ta in tray_actions:
            ...
            if ta_action == "ACCEPT":
                ...
                accepted_trays.append(
                    {"tray_id": tid, "qty": tray_qty, "is_top": is_top}
                )
            elif ta_action == "REJECT":
                slot_qty = int(ta.get("qty") or 0)
                rejected_trays.append(
                    {"tray_id": tid, "qty": slot_qty, "is_top": False}
                )
        ...
        if accepted_trays:
            ...
            if top_count != 1:
                return JsonResponse(
                    {
                        "success": False,
                        "error": f"Exactly one accepted tray must be marked as top (found {top_count})",
                    },
                    status=400,
                )
        ...
        for rt in rejected_trays:
            NickelQcTrayId.objects.filter(
                tray_id=rt["tray_id"], lot_id=unload_lot_id
            ).update(rejected_tray=True)
        ...
        if accepted_trays:
            ...
            for at in accepted_trays:
                if at["is_top"]:
                    NickelQcTrayId.objects.filter(
                        lot_id=unload_lot_id, tray_id=at["tray_id"]
                    ).update(top_tray=True, tray_quantity=at["qty"])
        ...
        if submission_type == "FULL_ACCEPT":
            ...
            jig_rec.next_process_module = "Nickel Audit"
            jig_rec.last_process_module = "Nickel QC"
            NickelQcTrayId.objects.filter(lot_id=unload_lot_id).update(
                rejected_tray=True
            )
        ...
        logger.info(
            f"[NQ_ACTION:PROCESS] lot_id={lot_id}, unload={unload_lot_id}, type={submission_type}, accepted={accepted_qty}, rejected={rejected_qty}"
        )
